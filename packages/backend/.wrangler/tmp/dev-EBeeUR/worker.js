...
// .wrangler/tmp/bundle-xCDAUt/checked-fetch.js
...
// ../chop-url-lib/dist/index.js
var require_dist = __commonJS({
  "../chop-url-lib/dist/index.js"(exports) {
    "use strict";
    init_checked_fetch();
    ...
    var ChopUrl2 = class {
      ...
      async createShortUrl(url) {
       ...
       VALUES (?, ?, ?, 0)`).bind(shortId, url, now.toISOString()).run();
        return {
          shortId,
          originalUrl: url,
      ...
      async getOriginalUrl(shortId) {
        const result = await this.db.prepare(`SELECT original_url FROM urls WHERE short_id = ?`).bind(shortId).first();
        if (!result) {
          throw new Error("URL not found");
        }
        await this.db.prepare(`UPDATE urls SET visits = visits + 1 WHERE short_id = ?`).bind(shortId).run();
        return result.original_url;
      }
      async getUrlInfo(shortId) {
        const result = await this.db.prepare(`SELECT * FROM urls WHERE short_id = ?`).bind(shortId).first();
        if (!result) {
          throw new Error("URL not found");
        }
        return {
    ...
    __name(ChopUrl2, "ChopUrl");
    exports.ChopUrl = ChopUrl2;
  }
});

// .wrangler/tmp/bundle-xCDAUt/middleware-loader.entry.ts
init_checked_fetch();

// .wrangler/tmp/bundle-xCDAUt/middleware-insertion-facade.js
init_checked_fetch();
